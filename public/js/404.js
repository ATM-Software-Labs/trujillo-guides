(function () {
  ...
  if (userMatch && userMatch[1]) {
    ...
    window.location.replace('/u.html?u=' + encodeURIComponent(rawHandle) + (window.location.hash || ''));
    return;
  }
  if (decodedPath === '/u' || decodedPath === '/u/') {
    try { sessionStorage.setItem('atm_resolved_404', '1'); } catch (e) {}
    window.location.replace('/u.html' + (window.location.hash || ''));
    return;
  }
  ...
  if (/^\/settings(?:\/|$)/i.test(decodedPath)) {
    try { sessionStorage.setItem('atm_resolved_404', '1'); } catch (e) {}
    window.location.replace('/settings.html' + (window.location.search || '') + (window.location.hash || ''));
    return;
  }
  ...
  if (guideMatch && guideMatch[1]) {
    try { sessionStorage.setItem('atm_resolved_404', '1'); } catch (e) {}
    window.location.replace('/g.html?id=' + encodeURIComponent(guideMatch[1]) + (window.location.hash || ''));
    return;
  }
})();
